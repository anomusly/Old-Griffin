...
const check = parseInt(message.guild.premiumTier.split("_")[1]);
    if(message.guild.premiumTier === "NONE") return message.channel.send({embeds: [new MessageEmbed().setColor(client.color).setDescription(`<a:Cross:1346485804368789525> | Your Server Doesn't Meet The **Roleicon Requirements**. Servers with level **2** boosts are allowed to set role icons`)]})
    
    if(check < 2)
      return message.channel.send({embeds: [new MessageEmbed().setColor(client.color).setDescription(`<a:Cross:1346485804368789525> | Your Server Doesn't Meet The **Roleicon** Requirements. Servers with level **2** boosts are allowed to set role icons`)]})
 if(!args[0]) return message.channel.send({embeds:[new MessageEmbed().setColor(client.color).setDescription(`<a:Cross:1346485804368789525> | ${message.guild.prefix}roleicon <role> <emoji>`)]});
    const role = message.mentions.roles.first() || message.guild.roles.cache.get(args[0]);
    if(!role) return message.channel.send({embeds:[new MessageEmbed().setColor(client.color).setDescription(`<a:Cross:1346485804368789525> | Provide valid role.`)]})
    if(role && !role.editable) {
        return message.channel.send({embeds:[new MessageEmbed().setColor(client.color).setDescription(`<a:Cross:1346485804368789525> | ${role} is having higher position than my role`)]});    
    }
    ...
    if(!args[1].match(emojiRegex)) return message.channel.send({embeds:[new MessageEmbed().setColor(client.color).setDescription(`<a:Cross:1346485804368789525> | Provide provide a **valid** emoji.`)]})
